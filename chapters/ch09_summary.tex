...
\begin{enumerate}
    \item At the evaluated low-SNR points, the best learned relays outperform amplify-and-forward (AF), where their learned soft denoising avoids AF's noise amplification (H1).
...
H4 & Equal parameter count eliminates architecture-dependent differences & \textbf{Rejected at low SNR; high-SNR convergence not seed-robust} (Table~\ref{tbl:seed-spread-3k}): at 3K params the six architectures span 4.1\% BER at 20~dB but 17.4\% at 0~dB, splitting into feedforward and sequence groups. Equalizing the budget required changing width, depth, state dimension and input window jointly (Table~\ref{tbl:table28}), so this equalizes capacity, not the other architectural degrees of freedom \\
