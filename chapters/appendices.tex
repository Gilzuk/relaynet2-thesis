...
The bound here is the bit-wise MAP detector, computed from the per-bit posteriors a BCJR/APP recursion delivers, and the distinction from MLSE is not pedantic: bit-wise MAP minimizes \emph{bit} error probability (for BPSK it coincides with symbol-MAP; for Gray-coded QPSK it is the per-bit marginal of the APPs), so no learned function of the same observation can improve on it on this thesis's BER metric, whereas MLSE minimizes \emph{sequence} error probability and is \emph{not} BER-optimal \cite{Forney1972MLSE, BahlCockeJelinekRaviv1974BCJR, ProakisSalehi2008DigitalComm}. Against genie-CSI MLSE the statement that the relay never wins is therefore an \emph{empirical} finding of Chapter~\ref{sec:unknown-channels}, not a theorem -- a learned relay beating Viterbi on BER would not contradict any optimality result. The BER-optimal comparison at QPSK (Section~\ref{sec:bcjr-benchmark}) confirms it holds there too: BCJR/APP is never worse than MLSE, with the advantage unresolvable above 16~dB; it remains unmeasured at BPSK, though the same $I/Q$ factorisation applies there by construction. The learned-receiver literature is consistent with this reading, its reported wins over ``classical'' receivers all being wins over mismatched or suboptimal baselines: the sliding bidirectional RNN passes full-CSI Viterbi only when CSI is imperfect \cite{FarsadGoldsmith2018NNDetection}; ViterbiNet gains only under CSI uncertainty \cite{Shlezinger2020ViterbiNet}; DeepRx outperforms LMMSE chains that are not MAP-optimal under the studied impairments \cite{Honkala2021DeepRx}; Ye et al.'s OFDM detector \cite{YeLiJuang2018OFDMDL} beats MMSE under pilot shortage, cyclic-prefix removal and clipping. End-to-end autoencoders \cite{OSheaHoydis2017PhysicalLayer} beat classical \emph{schemes} by redesigning the transmitter, not as a receiver-only counterexample.
...
\subsection{\texorpdfstring{Complexity: the $M^{L}$ trellis against a fixed forward pass} {Complexity: the M^L trellis against a fixed forward pass}}
MLSE maintains $M^{L-1}$ trellis states and evaluates $M^{L}$ branch metrics per symbol \cite{Forney1972MLSE, ProakisSalehi2008DigitalComm}. The deployed relay, an 11-sample window into 13 hidden units ($11 \to 13 \to 1$, $170$ parameters), costs approximately $2(11 \cdot 13 + 13)$ multiply--accumulates plus activations, about 330 floating-point operations per symbol, constant for the deployed architecture -- though spanning longer memory generally widens the window (Section~\ref{sec:complexity-viterbi-mlp}), after which the cost grows roughly linearly in it. For fairness: reduced-state sequence estimation \cite{EyuboqluQureshi1988RSSE} and related decision-feedback trellis methods also break the $M^{L}$ scaling, so full Viterbi is the steepest classical comparator, not the only one.

\subsection{The blind regime: CMA converges where decision-directed MLSE does not}

The constant modulus algorithm performs blind equalization of constant-modulus signals \cite{Godard1980SelfRecovering, TreichlerAgee1983CMA}. The study channel is minimum phase (zeros at $|z| \approx 0.707$), so a short FIR equalizer can approximately invert it; finite equalizer length and noise enhancement leave a residual BER of order $10^{-3}$ at 20~dB, consistent with the measured $3.3\times10^{-3}$ of Section~\ref{sec:blind-channel}, and known ill-convergence of Godard equalizers when too short \cite{Ding1991IllConvergence} matches the observed ``matches but does not excel'' behaviour. Decision-directed blind MLSE, a crude form of per-survivor processing that bootstraps channel estimates from its own decisions \cite{RaheliPolydorosTzou1995PSP}, has documented failure modes -- misconvergence, sign and shift ambiguities, error propagation -- matching the instability observed.

\subsection{The pilot-budget crossover: reliable at ten pilots, collapse at five}

Least-squares estimation of three unknown taps is identifiable from five pilots, so the collapse of Section~\ref{sec:partial-posterior} is not one of strict non-identifiability: convolution edge effects instead leave a near-square, ill-conditioned system whose least-squares error variance, proportional to $\sigma^{2}\,\mathrm{tr}\!\left((\mathbf{X}^{H}\mathbf{X})^{-1}\right)$, grows without useful bound, and Viterbi run with badly wrong taps propagates errors catastrophically -- consistent with the Cram\'er--Rao bound $\sigma^{2}L/N_{p}$ for this estimation problem \cite{Kay1993Estimation}. The result is specific to the classical LS-plus-Viterbi pipeline: meta-learned demodulators adapt from very few pilots \cite{ParkSimeone2021MetaDemod}, which bounds the claim's scope to the pipeline stated rather than contradicting it.

\subsection{Genie-CSI MLSE leads the minimal relay by only 1--1.5 dB}

A finite-window symbol-wise detector cannot do better than the MAP detector over that same window, which in turn cannot do better than the MAP detector given the whole received sequence: the window truncates the observation, and truncation cannot add information. The ordering is $P_e^{\mathrm{MAP,seq}} \le P_e^{\mathrm{MAP,window}} \le P_e^{\mathrm{any\ windowed\ detector}}$. The size of the finite-window penalty is channel-dependent and is not derived analytically here. That ordering is all theory supplies here -- how large the resulting penalty is on this channel is a quantitative question this thesis does not answer analytically, since no windowed-MAP detector was implemented, so the $1$--$1.5$~dB lost to genie-CSI Viterbi is not decomposed into the window's share and the 170-parameter approximation's. Published learned detectors close the gap further: the sliding bidirectional RNN comes within fractions of a dB of Viterbi \cite{FarsadGoldsmith2018NNDetection}, and ViterbiNet closes it essentially to zero given sufficient capacity and data \cite{Shlezinger2020ViterbiNet}. The residual gap in Section~\ref{sec:unknown-channel-experiment} is thus a property of the deliberately minimal parameter budget, not of learned detection as a method.
