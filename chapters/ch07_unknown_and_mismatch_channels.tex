...
The boundary of the claim is equally important. No statistically resolved advantage over matched MLSE is observed: given a reliable channel estimate, pilot-aided Viterbi remains ahead by approximately $1$--$1.5$~dB over the reported operating range. This is empirical, not an impossibility result, because MLSE minimizes sequence error rather than BER; a bit-MAP/BCJR receiver would be the BER-optimal matched comparator and was not evaluated. The learned relay's advantage is therefore not demonstrated superior detection but the absence of a per-block channel-estimation precondition. Where the channel is fixed and known to both, it is simply the cheaper detector, constant and parallelizable per symbol against the trellis's $M^{L-1}$; where the channel must be identified and cannot be, it is the one that still functions, needing neither a per-block estimate nor the channel family. Where classical processing can identify the channel it should be used; where it cannot, the learned relay is what still works.
...
Two controls run first and gate the comparison, because the benchmark withdrawn in Section~\ref{sec:qpsk-unknown-channel} was one that looked plausible and was model-mismatched. With the fading removed the two detectors agree ($0.021424$ against $0.021764$ at 8~dB, a paired difference of $+0.00034 \pm 0.00016$; both exactly zero at 20~dB), and with the ISI removed the BER-optimal rule reduces to the nearest-symbol slicer on every one of $10{,}000$ symbols.
