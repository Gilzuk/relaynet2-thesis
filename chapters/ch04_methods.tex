...
\paragraph{Why per-symbol processing suffices here.} Under the canonical model the current observation $y_R[i]$ is already a sufficient statistic for detecting $x[i]$, so no relay comparison in the canonical study requires sequence detection and the classical AF and DF baselines operate per symbol. Some neural architectures nevertheless receive a finite window of neighboring observations (Section~\ref{sec:relay-strategies}); this is an architectural input convention for the evaluated feedforward and sequence models, and under the canonical i.i.d.\ assumptions it neither introduces channel memory nor supplies additional information about $x[i]$.
