...
The system under study is a two-hop relay network with a single relay node. All methods in this chapter are described for the canonical setup of Chapter~\ref{sec:introduction}: SISO on both hops, i.i.d.\ Rayleigh fast fading, complex baseband, QPSK, uncoded BER. \emph{Fast fading} here means the strict memoryless case, $h[i] \sim \mathcal{CN}(0,1)$ drawn independently for every symbol --- i.i.d.\ per-symbol Rayleigh fading, not a fading process with a finite Doppler spectrum. Every later use of the term in this thesis carries that meaning. QPSK on the canonical Rayleigh channel is the pairing used here and throughout the canonical comparison; the coded and unknown-channel studies additionally use the pairings set out in Table~\ref{tbl:configurations}.

\paragraph{Per-symbol information.} In the canonical memoryless channel, neighboring observations add no information about the current symbol conditional on its receiver-side CSI. The equalized observation preserves the local hard BPSK/QPSK decision boundary. For soft inference, however, the channel magnitude also specifies reliability; discarding it does not preserve the full CSI-conditioned posterior. Learned canonical relays consume equalized windows without explicit channel magnitudes, while the coherent front end still uses CSI.
...
\begin{longtable}[]{@{}p{0.18\linewidth}p{0.19\linewidth}p{0.14\linewidth}p{0.40\linewidth}@{}}
...
\textbf{A consequence worth stating plainly.} The channel functions set \(N_0\) from the average \emph{symbol} energy, so the reported SNR axis is \(E_s/N_0\). For real BPSK, \(E_s=E_b\); for QPSK, \(E_b=E_s/2\), so its bit-energy label is \(3.01\)~dB below the plotted value. The real branches use variance \(N_0/2\) and the complex branches split the same \(N_0\) equally across I and Q. Where a BER figure is read against a BPSK closed form, this conversion is stated explicitly at the point of use.

\textbf{Power Normalization.} All relay strategies normalize their output power to ensure a common empirical transmit-power constraint:
...
Here \(P_{\text{current}}\) is estimated over the complete simulated relay-output block. The normalization is therefore block-adaptive and noncausal at the symbol level; it is appropriate to the block-buffered simulation but should not be read as an online gain known before the block has been observed.
...
\begin{equation}\protect\phantomsection\label{eq:relay-received-siso}{\tilde{y}_R[i] = x[i] + \frac{n_1[i]}{h_1[i]}, \quad \frac{n_1[i]}{h_1[i]} \sim \mathcal{CN}\!\left(0, \frac{\sigma^2}{|h_1[i]|^2}\right)
}\end{equation}
...
\begin{enumerate}
\item \textbf{Classical AF:} Block-adaptive power normalization as in Equation~\ref{eq:af-gain}, preserving the received signal and its noise. The CSI-assisted analytical SNR expression in Equation~\ref{eq:af-snr} is background, not an exact prediction of this empirical block-normalization implementation.
...
\textbf{Real network on a complex channel.} The network above is real-valued, while the canonical channel and its QPSK constellation are complex. The two are reconciled by operating \emph{per axis}: the physical QPSK components are $\pm1/\sqrt{2}$, and it is convenient to express each in normalized binary coordinates, $u_I=\sqrt{2}\Re\{\tilde y\}$ and $u_Q=\sqrt{2}\Im\{\tilde y\}$, whose clean targets are $\pm1$. The identical 169-parameter network is applied to both normalized streams and the outputs are recombined as $\hat{x}_R=(\hat{u}_I+j\hat{u}_Q)/\sqrt{2}$ before the common power-normalization step. The parameter count is unchanged by this --- one set of weights is shared across both axes, which is what makes the two sub-problems the \emph{same} problem --- but the arithmetic cost per transmitted symbol is twice the per-window figure. Every learned relay in the canonical comparison of Chapter~\ref{sec:experiments} (MLP, hybrid, VAE, cGAN and the sequence models) is applied this way; AF and symbol-wise DF operate on the complex sample directly.
...
\item  \textbf{Generative cGAN (WGAN-GP)} \cite{MirzaOsindero2014cGAN,Gulrajani2017WGANgp}\textbf{:} Adversarial relay with a generator conditioned on the noisy signal and a critic providing the training signal. Generator: \((7+8 \to 32 \to 32 \to 16 \to 1)\), Critic: \((1+7 \to 32 \to 16 \to 1)\). Total: 2,946 parameters. Trained with Wasserstein loss, gradient penalty (\(\lambda=10\)), and L1 reconstruction loss (\(\lambda_{\text{L1}}=100\)) for 200 epochs.
...
where \(\mathbb{1}(\cdot)\) is the indicator function and \(N = 10{,}000\) is the number of bits per trial. Under independent Bernoulli errors, the per-trial variance is \(P_e(1-P_e)/N\). Window overlap and block normalization can invalidate that independence; the reported trial-level intervals instead use variation across independent simulation trials.

\textbf{Confidence Intervals.} The 95\% confidence interval is computed from the \(M = 10\) independent trial estimates as:

\begin{equation}\protect\phantomsection\label{eq:confidence-interval}{\text{CI}_{95\%} = \bar{P}_e \pm t_{0.975, M-1} \cdot \frac{s}{\sqrt{M}}
}\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad confidence-interval}

where $\bar P_e$ and $s$ are the mean and sample standard deviation of ten trial BERs, and $t_{0.975,9}=2.262$. Sparse errors can make the approximation unreliable. A zero-width interval or apparent curve convergence at the measurement floor does not establish stable ranking.

\textbf{BER resolution and dependence.} With 100{,}000 aggregate bits, one observed error changes the pooled estimate by $10^{-5}$; an observed zero is not evidence of zero BER. For a fixed budget of independent Bernoulli errors, the exact one-sided 95\% zero-error upper bound is $1-0.05^{1/n}\approx3/n$ \cite{HanleyLippmanHand1983ZeroNumerators}. That assumption does not follow from independent fading alone: overlapping windows, shared I/Q reliability, coding and whole-block normalization can correlate bit errors. Use independent blocks or trials as the uncertainty unit, and separate training-seed replication. A zero-width trial interval at zero observed errors is uninformative. Historical first-error-dependent exposures in Chapter~\ref{sec:unknown-channels} require separate validation and do not inherit fixed-budget binomial coverage.

\textbf{Per-experiment simulation budgets.} The canonical and normalized comparisons use ten trials of 10{,}000 bits per SNR. The original unknown-ISI, flat-channel and QPSK studies use ten trials of 100{,}000 bits; the composite study uses the same unique-trial budget, with its historical aggregation caveat in Section~\ref{sec:composite-channel}. Blind and pilot studies use 50 trials of 20{,}000 bits. Training replication and high-budget confirmations are additional, separately declared units. The coded-memory precision sweep uses five seeds; other coded tests use 100 paired trials. No single trial count or significance resolution applies to every experiment. For a two-sided exact Wilcoxon test with five nonzero pairs, the smallest attainable $p$ is $0.0625$, so those five-seed measurements cannot support a $p<0.05$ claim from that test.
...
\begin{equation}\protect\phantomsection\label{eq:wilcoxon-test}{H_0: \text{the paired-difference distribution is symmetric about }0
}\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad wilcoxon-test}

The Wilcoxon signed-rank results are exploratory paired comparisons. Their location interpretation requires assumptions on the paired-difference distribution, and failure to reject is not equivalence. No multiplicity correction was applied to the post-inspection comparisons, so the reported $p$-values are descriptive, per-comparison quantities. Original fixed-checkpoint Monte Carlo intervals do not include retraining variance. Separate three-initialization studies cover the size sweep, native and equal-budget architectures, and several unknown-channel experiments; those sections state their own aggregation rules. Initialization spread is strongly configuration-dependent, so the pooled median $0.180$~dB is not a universal resolution threshold.

\subsection{Training Protocol}\label{sec:training-protocol}

In the original canonical comparisons, each AI relay is trained \textbf{once} at the beginning of the experiment using: - \textbf{Multi-SNR training data:} Signals generated at SNR = 5, 10, 15 dB in equal proportions - \textbf{Training samples:} 25,000 (supervised), 20,000 (generative), 10,000 (sequence models) - \textbf{Single training per channel:} The same trained model is evaluated across all SNR points

Multi-SNR training was chosen to expose the relay to several noise levels. No single-SNR versus multi-SNR ablation was performed, so improved generalization from this choice is a design rationale rather than an experimentally isolated result.

\textbf{Sparse SNR training grid.} For training SNRs $\{5,10,15\}$~dB and testing at $0,2,\ldots,20$~dB, the in-range test points are $6,8,10,12,14$~dB; $0,2,4,16,18,20$~dB lie outside the training span. This identifies interpolation and extrapolation ranges, but without a dense-grid training ablation it does not establish how much of any performance difference is caused by sparse SNR sampling.
...
\textbf{Weight initialization.} He initialization \cite{He2015DelvingRectifiers} is used for all layers with ReLU activation (\(W \sim \mathcal{N}(0, 2/n_{\text{in}})\)), while tanh layers use the fan-in scaling \(W \sim \mathcal{N}(0, 1/n_{\text{in}})\). The latter is the LeCun rule \cite{LeCun1998EfficientBackprop}, not Xavier/Glorot initialization, which scales by \(2/(n_{\text{in}} + n_{\text{out}})\) so as to preserve variance in the backward pass as well \cite{GlorotBengio2010Understanding}; earlier drafts named it Xavier, and the label was wrong rather than the code. Both schemes hold the forward activation variance roughly constant across layers, which aims to preserve signal scale at initialization but does not guarantee stable gradients throughout training.
...
Experiment drivers declare source, channel, and training seeds where replication was performed. Seed formulas differ by driver; the committed scripts and result metadata are the reproduction authority. The original sequence checkpoints did not control every training RNG, so they cannot be reconstructed exactly from a later seed declaration.
...
\textbf{Unified input window.} All 3K models use a window size of 11 (vs.~5 for original MLP/Hybrid, and 7 for original VAE/cGAN), providing a common input context. This removes one confound --- differences in the amount of input information available. It does not isolate architecture: as the parameter-normalization above records, meeting the shared budget also required adjusting width, depth, state dimension, head count and layer count together, so those degrees of freedom still co-vary with architecture. Any residual gap is therefore attributable to the remaining structural differences collectively, of which architecture is the most salient but not the only one.
...
\begin{equation}\protect\phantomsection\label{eq:iq-splitting}{\hat{x}_R = \frac{f_\theta(\sqrt{2}\,\text{Re}(y_R)) + j f_\theta(\sqrt{2}\,\text{Im}(y_R))}{\sqrt{2}}
}\end{equation}
