...
\begin{enumerate}
    \item \textbf{Gap 1: Limited cross-paradigm relay comparison.} The cited work studies related receiver or end-to-end tasks, generally with one architecture family at a time: Ye et al.~\cite{YeLiJuang2018OFDMDL} studied DNNs for OFDM detection, Dorner et al.~\cite{Dorner2018OverTheAirDL} explored end-to-end autoencoders, and Samuel et al.~\cite{SamuelDiskinWunder2019MIMODetect} applied unfolded networks to MIMO detection. The reviewed literature did not identify a comparison of feedforward networks, variational autoencoders, attention-based Transformers, and state-space models (Mamba \cite{gu2023mamba}, Mamba-2 SSD \cite{DaoGu2024TransformersSSM}) on the same relay-denoising task. The cGAN is implemented here but excluded from the reported comparison, so the adversarial paradigm is not quantitatively covered.
