...
reproduced. \textbf{Transformers} \cite{Vaswani2017Attention} use multi-head self-attention: every position
attends to every other through learned query, key and value projections, giving a global receptive field at
...
reformulates the same recurrence through its duality with a structured attention matrix, which buys a faster
implementation rather than a different function class. The classical analogue is adaptive filtering, and that
...
(Section~\ref{sec:bcjr-benchmark} measures this directly); and a rate-$k/n$ code over a constraint-length-$K$
shift register has a $2^{K-1}$-state trellis. This thesis uses $k=1$, $n=2$, $K=3$ throughout, and obtains
rates $2/3$ and $3/4$ from that mother code by puncturing.
