...
% bidi's table patch references the legacy array tagging hook. TeX Live 2026
% no longer defines it for an untagged document; keep older releases working.
...
\usepackage{setspace}
\onehalfspacing
% Permit modest extra interword flexibility without altering type or margins.
...
\usepackage{ragged2e}
% Explicit dependency so source bundles include the local calendar shim.
...
\ifXeTeX
  \newfontfamily\hebrewfont[Script=Hebrew, Path=./fonts/, Extension=.otf,
    UprightFont=*-Medium, BoldFont=*-Bold,
    ItalicFont=*-MediumItalic, BoldItalicFont=*-BoldItalic]{DavidCLM}
...
\setcounter{handoverfrontpage}{\value{page}}
\mainmatter
...
\include{chapters/ch07_unknown_and_mismatch_channels}
\include{chapters/ch08_discussion}
\include{chapters/ch09_summary}
...
\clearpage
\pagenumbering{roman}
\setcounter{page}{\value{handoverfrontpage}}
\input{chapters/hebrew_abstract}
\clearpage
\hypersetup{pageanchor=false}
...
{\large בית הספר להנדסת חשמל ומחשבים}\\[1.5cm]
...
{\large בהנדסת חשמל ואלקטרוניקה באוניברסיטת תל-אביב}\\[1.5cm]

{\large על-ידי}\\[6pt]
{\Large\bfseries גיל צוקרמן}\\[1.5cm]

{\small העבודה נעשתה בבית הספר להנדסת חשמל ומחשבים \\
בהדרכת ד"ר אנטולי חינה ופרופ' ראג'א ג'יריס}\\
 
\vfill

{\large\textbf{ספטמבר 2026}}\\[1.5cm]
